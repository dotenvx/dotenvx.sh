...
#  |                                                                                                 |
#  |  ```sh                                                                                          |
#  |  curl -sfS https://dotenvx.sh/radar > install.sh                                                |
#  |  chmod +x install.sh                                                                            |
#  |  ./install.sh                                                                                   |
#  |  ```                                                                                            |
#  |                                                                                                 |
...
#  |  ```sh                                                                                          |
#  |  # curl examples                                                                                |
#  |  curl -sfS "https://dotenvx.sh/radar" | sudo sh                                                 |
#  |  curl -sfS "https://dotenvx.sh/radar?version=0.44.5" | sh                                       |
#  |  curl -sfS "https://dotenvx.sh/radar?directory=." | sh                                          |
#  |  curl -sfS "https://dotenvx.sh/radar?directory=/custom/path&version=0.44.5" | sh                |
#  |                                                                                                 |
...
#  |  ```                                                                                            |
#  |                                                                                                 |
#  |  see [`dotenvx-radar`](https://github.com/dotenvx/dotenvx-radar) for usage.                     |
#  |                                                                                                 |
...
usage() {
  ...
  echo ""
  echo "install dotenvx-radar"
  echo ""
  echo "Options:"
  echo "  --directory       directory to install dotenvx-radar to (default: \"/usr/local/bin\")"
  echo "  --version         version of dotenvx-radar to install (default: \"$VERSION\")"
  echo ""
  echo "Commands:"
  echo "  install           install dotenvx-radar"
...
is_os_supported() {
  ...
  *)
    ...
    echo "? request support by opening an issue at [https://github.com/dotenvx/dotenvx-radar/issues]"
...
is_installed() {
  ...
  echo "[dotenvx-radar@$current_version] already installed ($(directory)/$(binary_name))"
...
filename() {
  echo "dotenvx-radar-$VERSION-$(os_arch).tar.gz"
...
which_path() {
  local result
  result=$(command -v dotenvx-radar 2>/dev/null) # capture the output without displaying it on the screen
...
warn_of_any_conflict() {
  local dotenvx_radar_path="$(which_path)"

  if [ "$dotenvx_radar_path" != "" ] && [ "$dotenvx_radar_path" != "$(directory)/$(binary_name)" ]; then
    echo "[DOTENVX_RADAR_CONFLICT] conflicting dotenvx-radar found at $dotenvx_radar_path" >&2
...
install() {
  ...
  # let user know
  echo "[dotenvx-radar@$VERSION] installed successfully ($(directory)/$(binary_name))"
  echo "now type: dotenvx-radar help"
...
run() {
  ...
  if [ -n "$VERSION" ]; then
    ...
    if is_installed "$VERSION"; then
      echo "[dotenvx-radar@$VERSION] already installed ($(directory)/$(binary_name))"
...
# "thanks for using dotenvx-radar!" - mot
