...
INSTALL_SCRIPT_URL="https://dotenvx.sh/armor"
FORCE=""
...
#  |                                                                                                 |
#  |  ```sh                                                                                          |
#  |  curl -sfS https://dotenvx.sh/armor > install.sh                                                |
#  |  chmod +x install.sh                                                                            |
#  |  ./install.sh                                                                                   |
#  |  ```                                                                                            |
#  |                                                                                                 |
...
#  |  ```sh                                                                                          |
#  |  # curl examples                                                                                |
#  |  curl -sfS "https://dotenvx.sh/armor" | sudo sh                                                 |
#  |  curl -sfS "https://dotenvx.sh/armor?version=0.44.5" | sh                                       |
#  |  curl -sfS "https://dotenvx.sh/armor?directory=." | sh                                          |
#  |  curl -sfS "https://dotenvx.sh/armor?directory=/custom/path&version=0.44.5" | sh                  |
#  |                                                                                                 |
...
filename() {
  echo "dotenvx-armor-$VERSION-$(os_arch).tar.gz"

  return 0
}

download_url() {
  echo "$REGISTRY_URL/@dotenvx/dotenvx-armor-$(os_arch)/-/dotenvx-armor-$(os_arch)-$VERSION.tgz"
...
which_path() {
  local result
  result=$(command -v dotenvx-armor 2>/dev/null) # capture the output without displaying it on the screen
...
warn_of_any_conflict() {
  local dotenvx_armor_path="$(which_path)"

  if [ "$dotenvx_armor_path" != "" ] && [ "$dotenvx_armor_path" != "$(directory)/$(binary_name)" ]; then
    echo "[DOTENVX_ARMOR_CONFLICT] conflicting dotenvx-armor found at $dotenvx_armor_path" >&2
