...
INSTALL_SCRIPT_URL="https://dotenvx.sh/ext/vault"
...
#  |                                                                                                 |
#  |  ```sh                                                                                          |
#  |  curl -sfS https://dotenvx.sh/ext/vault > install.sh                                            |
#  |  chmod +x install.sh                                                                            |
#  |  ./install.sh                                                                                   |
#  |  ```                                                                                            |
#  |                                                                                                 |
...
#  |  curl -sfS "https://dotenvx.sh/ext/vault?version=0.44.5" | sh                                   |
#  |  curl -sfS "https://dotenvx.sh/ext/vault?directory=." | sh                                      |
#  |  curl -sfS "https://dotenvx.sh/ext/vault?directory=/custom/path&version=0.44.5" | sh            |
#  |                                                                                                 |
...
#  |  ```                                                                                            |
#  |                                                                                                 |
#  |  see [`dotenvx-ext-vault`](https://github.com/dotenvx/dotenvx-ext-vault) for usage.             |
#  |                                                                                                 |
...
usage() {
  ...
  echo ""
  echo "install dotenvx-ext-vault"
  echo ""
  echo "Options:"
  echo "  --directory       directory to install dotenvx-ext-vault to (default: \"/usr/local/bin\")"
  echo "  --version         version of dotenvx-ext-vault to install (default: \"$VERSION\")"
  echo ""
  echo "Commands:"
  echo "  install           install dotenvx-ext-vault"
...
is_os_supported() {
  ...
  *)
    ...
    echo "? request support by opening an issue at [https://github.com/dotenvx/dotenvx-ext-vault/issues]"
...
is_installed() {
  ...
  echo "[dotenvx-ext-vault@$current_version] already installed ($(directory)/$(binary_name))"
...
filename() {
  echo "dotenvx-ext-vault-$VERSION-$(os_arch).tar.gz"

  return 0
}

download_url() {
  echo "$REGISTRY_URL/@dotenvx/dotenvx-ext-vault-$(os_arch)/-/dotenvx-ext-vault-$(os_arch)-$VERSION.tgz"
...
binary_name() {
  if $(is_windows); then
    echo "dotenvx-ext-vault.exe"
  else
    echo "dotenvx-ext-vault"
  fi
...
which_path() {
  local result
  result=$(command -v dotenvx-ext-vault 2>/dev/null) # capture the output without displaying it on the screen
...
warn_of_any_conflict() {
  ...
  if [ "$dotenvx_path" != "" ] && [ "$dotenvx_path" != "$(directory)/$(binary_name)" ]; then
    echo "[DOTENVX_CONFLICT] conflicting dotenvx-ext-vault found at $dotenvx_path" >&2
...
install() {
  ...
  # let user know
  echo "[dotenvx-ext-vault@$VERSION] installed successfully ($(directory)/$(binary_name))"
  echo "now type: dotenvx-ext-vault help"
...
run() {
  ...
  if [ -n "$VERSION" ]; then
    ...
    if is_installed "$VERSION"; then
      echo "[dotenvx-ext-vault@$VERSION] already installed ($(directory)/$(binary_name))"
...
# "thanks for using dotenvx-ext-vault!" - mot
