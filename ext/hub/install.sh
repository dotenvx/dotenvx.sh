...
INSTALL_SCRIPT_URL="https://dotenvx.sh/ext/hub"
...
#  |                                                                                                 |
#  |  ```sh                                                                                          |
#  |  curl -sfS https://dotenvx.sh/ext/hub > install.sh                                              |
#  |  chmod +x install.sh                                                                            |
#  |  ./install.sh                                                                                   |
#  |  ```                                                                                            |
#  |                                                                                                 |
...
#  |  ```sh                                                                                          |
#  |  # curl examples                                                                                |
#  |  curl -sfS "https://dotenvx.sh/ext/hub" | sudo sh                                               |
#  |  curl -sfS "https://dotenvx.sh/ext/hub?version=0.44.5" | sh                                     |
#  |  curl -sfS "https://dotenvx.sh/ext/hub?directory=." | sh                                        |
#  |  curl -sfS "https://dotenvx.sh/ext/hub?directory=/custom/path&version=0.44.5" | sh              |
#  |                                                                                                 |
...
#  |  ```                                                                                            |
#  |                                                                                                 |
#  |  see [`dotenvx-ext-hub`](https://github.com/dotenvx/dotenvx-ext-hub) for usage.                 |
#  |                                                                                                 |
...
usage() {
  ...
  echo ""
  echo "install dotenvx-ext-hub"
  echo ""
  echo "Options:"
  echo "  --directory       directory to install dotenvx-ext-hub to (default: \"/usr/local/bin\")"
  echo "  --version         version of dotenvx-ext-hub to install (default: \"$VERSION\")"
  echo ""
  echo "Commands:"
  echo "  install           install dotenvx-ext-hub"
...
is_os_supported() {
  ...
  *)
    ...
    echo "? request support by opening an issue at [https://github.com/dotenvx/dotenvx-ext-hub/issues]"
...
is_installed() {
  ...
  echo "[dotenvx-ext-hub@$current_version] already installed ($(directory)/$(binary_name))"
...
filename() {
  echo "dotenvx-ext-hub-$VERSION-$(os_arch).tar.gz"

  return 0
}

download_url() {
  echo "$REGISTRY_URL/@dotenvx/dotenvx-ext-hub-$(os_arch)/-/dotenvx-ext-hub-$(os_arch)-$VERSION.tgz"
...
binary_name() {
  if $(is_windows); then
    echo "dotenvx-ext-hub.exe"
  else
    echo "dotenvx-ext-hub"
  fi
...
which_path() {
  local result
  result=$(command -v dotenvx-ext-hub 2>/dev/null) # capture the output without displaying it on the screen
...
warn_of_any_conflict() {
  ...
  if [ "$dotenvx_path" != "" ] && [ "$dotenvx_path" != "$(directory)/$(binary_name)" ]; then
    echo "[DOTENVX_CONFLICT] conflicting dotenvx-ext-hub found at $dotenvx_path" >&2
...
install() {
  ...
  # let user know
  echo "[dotenvx-ext-hub@$VERSION] installed successfully ($(directory)/$(binary_name))"
  echo "now type: dotenvx-ext-hub help"
...
run() {
  ...
  if [ -n "$VERSION" ]; then
    ...
    if is_installed "$VERSION"; then
      echo "[dotenvx-ext-hub@$VERSION] already installed ($(directory)/$(binary_name))"
...
# "thanks for using dotenvx-ext-hub!" - mot
